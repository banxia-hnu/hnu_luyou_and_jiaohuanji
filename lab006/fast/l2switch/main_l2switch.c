...
{
	printf("pkt_send_normal->%p,port:%d,len:%d\n",pkt,pkt->um.inout,pkt_len);
	fast_ua_send(pkt,pkt_len);
}

void pkt_send_flood(struct fast_packet *pkt,int pkt_len)
{
	int i = 0,inport = pkt->um.inout;/*保存输入端口*/
...
void *nm08_mac_aging(void *argv)
{
	...
	while(1)
	{
		...
		sleep(10);/*老化时间误差，每10秒才判断一次。如果提高精度可缩短时间*/
		nm08_show_mac_info();
	}
}
...
int callback(struct fast_packet *pkt,int pkt_len)
{
	...
	printf("port:%d,dstmid:%d,len:%d,dmac:%02X:%02X:%02X:%02X:%02X:%02X,smac:%02X:%02X:%02X:%02X:%02X:%02X\n",pkt->um.inout,pkt->um.dstmid,pkt_len,
	...
	/*MAC地址学习过程*/
	learn_smac(pkt->um.inout,&pkt->data[MAC_LEN]);/*用源MAC位置开始学习*/

	/*查表过程*/
	outport = find_dmac(pkt->um.inout,pkt->data);/*用目的MAC地址开始查表*/
	...
	{
		pkt->um.inout = outport;/*修改报文输出端口号*/
...
void ua_init(void)
{
	...
	if((ret=fast_ua_init(1,callback)))//UA模块实例化(输入参数1:接收模块ID号,输入参数2:接收报文的回调处理函数)
	{
		perror("fast_ua_init!\n");
...
int main(int argc,char* argv[])
{
	...
	fast_reg_wr(FAST_ACTION_REG_ADDR|FAST_DEFAULT_RULE_ADDR,ACTION_SET_MID<<28|1);
